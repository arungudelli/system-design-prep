...
- **API gateway / edge / load balancer** — the most common place; reject early before requests reach app servers.
  (See [API gateway](02-systems/api-gateway) *(coming)*.)
...
- **API gateway** — the canonical home of rate limiting (per-key quotas). See [API gateway](02-systems/api-gateway) *(coming)*.
...
← Back to **[patterns](/)** · Related: [queues & workers (backpressure)](01-patterns/queues-workers.md) · [caching](01-patterns/caching.md)
