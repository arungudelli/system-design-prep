...
Full treatment in [idempotency](01-patterns/idempotency.md) *(coming)*. The one-liner: **at-least-once + idempotent
...
- **Throttling / rate limiting** at the producer (see [rate limiting](01-patterns/rate-limiting.md) *(coming)*).
...
**oldest-message age** (SLA for how stale processing may get). See [observability](01-patterns/observability.md) *(coming)*.
...
- **Push vs pull:** pull (SQS/Kafka) lets consumers control their own rate (natural backpressure); push
  (RabbitMQ) is lower-latency but can overwhelm a slow consumer. See [push vs pull](03-tradeoffs/push-vs-pull.md) *(coming)*.
...
← Back to **[patterns](/)** · Related: [idempotency](01-patterns/idempotency.md) *(coming)* · [caching](01-patterns/caching.md) · [sharding](01-patterns/sharding.md)
