...
- **At-least-once queues:** the broker redelivers if a worker doesn't ack in time (see
  [queues & workers](01-patterns/queues-workers.md#acknowledgment--the-visibility-timeout-the-core-mechanic)).
...
- Or use a **lease with a fencing token** so a stale worker's write is rejected (see
  [distributed locks](01-patterns/distributed-locks.md) *(coming)*).
...
[queues & workers](01-patterns/queues-workers.md#idempotency-the-essential-partner--retries-make-this-mandatory)
...
- **Transactional outbox** — pairs with idempotent consumers to make event delivery reliable
  ([outbox](01-patterns/queues-workers.md#the-transactional-outbox-dont-lose-this-one--its-a-top-follow-up)).
...
← Back to **[patterns](/)** · Related: [queues & workers](01-patterns/queues-workers.md) · [distributed locks](01-patterns/distributed-locks.md) *(coming)*
