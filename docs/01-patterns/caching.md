...
> split the key, salt it, or add a caching layer in front. See [sharding](sharding.md) *(coming)*.
...
- **CDN** for static/media — caching at the edge (see object storage & CDN *(coming)*).
...
← Back to **[patterns](../index.md)** · Related: [sharding](sharding.md) *(coming)* · [rate limiting](rate-limiting.md) *(coming)*
