...
follows from this boundary. See [architecture](architecture.md#the-core-insight-separate-scheduling-from-execution).
</details>
...
tiny bucket that shards across nodes. See [finding due jobs](deep-dives.md#1-finding-what-is-due-efficiently).
</details>
...
tokens** so a zombie worker can't double-apply. See [exactly-once](deep-dives.md#2-exactly-once-execution-the-honest-version).
</details>
...
enqueues. See [coordination](deep-dives.md#4-preventing-double-fire-across-schedulers-coordination).
</details>
...
minimizes the outage in the first place. See [failures](failure-scenarios.md#scheduler-crashes--is-down-at-a-fire-time).
</details>
...
each fire and store it. See [cron correctness](deep-dives.md#5-cron--time-correctness-subtle-interviewers-probe-it).
</details>
...
than restarts. See [long jobs](deep-dives.md#7-long-running-jobs).
</details>
...
the durable store. See [poll vs push](tradeoffs.md#1-poll-pull-vs-push-scheduling).
</details>
