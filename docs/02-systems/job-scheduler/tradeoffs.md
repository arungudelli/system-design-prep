...
scale. See [architecture](architecture.md#the-core-insight-separate-scheduling-from-execution).
...
**Choose:** **exactly-once effect.** Say it out loud; it's the mature answer. → [deep dive](deep-dives.md#2-exactly-once-execution-the-honest-version).
...
failover. They compose. → [coordination](deep-dives.md#4-preventing-double-fire-across-schedulers-coordination).
...
store regardless. → [data model](api-data-model.md#sql-or-nosql).
...
low-frequency business jobs). → [cron correctness](deep-dives.md#5-cron--time-correctness-subtle-interviewers-probe-it).
