...
[idempotency](../../01-patterns/idempotency.md), and distributed locks *(coming)* you've studied.
...
2. **[Capacity](capacity.md)** — jobs/sec, due-scan rate, storage
3. **[API & Data Model](api-data-model.md)** — job/schedule/run schemas + the "due index"
4. **[Architecture](architecture.md)** — scheduler + queue + workers, the fire loop, scaling
5. **[Deep Dives](deep-dives.md)** — finding due jobs, exactly-once, leases + fencing, cron, retries, long jobs
6. **[Trade-offs](tradeoffs.md)** — the decisions, both sides
7. **[Failure Scenarios](failure-scenarios.md)** — scheduler down at fire time, worker crash, dup fire
8. **[Interview Questions](interview-questions.md)** — attempt first, then reveal
9. **[Cheatsheet](cheatsheet.md)** — 1-page revision
10. **[★ Principal Deep Dive](principal-deep-dive.md)** — the **staged stops** (cron-on-a-box → sharded multi-region) + max-scale variant (time-bucketed sharding, timing wheels, multi-region, fairness) with a reconciliation table of when to adopt/remove each piece
