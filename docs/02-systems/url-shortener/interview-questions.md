...
must be unguessable, run the counter through a reversible permutation. See [Deep Dives](deep-dives.md#1-unique-short-code-generation-the-core-sub-problem).
</details>
...
async click event. Redirect is synchronous/latency-critical; analytics is async. See [Architecture](architecture.md#10-requestdata-flows).
</details>
...
writes/storage trivial → no sharding. See [Capacity](capacity.md).
</details>
