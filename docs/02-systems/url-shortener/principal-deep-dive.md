...
> **"what changes when the numbers get serious"** view. Read the base first: [Requirements](requirements.md)
> · [Capacity](capacity.md) · [Architecture](architecture.md)
...
- **Why not PostgreSQL as the mapping store?** A single Postgres primary is a **regional SPOF** and a write/HA
  ...
  is simpler and correct — see the base [trade-offs](tradeoffs.md#2-sql-vs-nosql). The SLA is what tips it.)*
...
See also the base [key-generation deep dive](deep-dives.md#1-unique-short-code-generation-the-core-sub-problem).
...
[Q15](interview-questions.md); at 10B/month + five-nines it's reasonable, and the KV store
does it natively.)
...
- **Why both:** lazy handles hot links cheaply; sweeping/TTL handles the long cold tail so storage + key space don't
  leak. See base [expiry deep dive](deep-dives.md#6-expiry--ttl).
...
← Back to **[URL Shortener overview](README.md)** · Base **[deep dives](deep-dives.md)** · **[cheatsheet](cheatsheet.md)**
