...
politeness is a local decision. Persist it (durable). See [frontier](deep-dives.md#1-the-url-frontier-the-heart-of-the-crawler).
</details>
...
crawled_at`) for exactness. **Normalize URLs first** or dedup is meaningless. See [dedup](deep-dives.md#2-url-deduplication-the-seen-set).
</details>
...
Hamming distance. Store one canonical copy. See [content dedup](deep-dives.md#3-content-deduplication-near-duplicate-detection).
</details>
...
and pattern blacklists. Detect via abnormal URLs-per-host growth. See [traps](deep-dives.md#6-crawler-traps--hostile-content-robustness).
</details>
...
changed. The frontier's priority + next-eligible time doubles as the recrawl scheduler. See [freshness](deep-dives.md#7-freshness--recrawl-turning-a-one-shot-into-a-living-index).
</details>
...
in-RAM queue. See [failures](failure-scenarios.md#frontier-node-fails).
</details>
