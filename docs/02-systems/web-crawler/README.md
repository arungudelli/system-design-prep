...
2. **[Capacity](capacity.md)** — pages/sec, storage, bandwidth, fetcher count
3. **[API & Data Model](api-data-model.md)** — internal interfaces + the frontier/dedup/store schemas
4. **[Architecture](architecture.md)** — components, the crawl loop, scaling
5. **[Deep Dives](deep-dives.md)** — URL frontier, dedup (Bloom + content hash), politeness, DNS, traps, freshness
...
10. **[★ Principal Deep Dive](principal-deep-dive.md)** — the **staged stops** (small → web-scale) + max-scale variant (multi-region, JS-render tier, ML priority, distributed dedup) with a reconciliation table of when to adopt/remove each piece
